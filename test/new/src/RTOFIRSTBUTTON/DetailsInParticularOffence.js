...
class OffenceInParticular extends Component{
    state={
        offence:[],
        penalty:0,
        numberOfOffence:0
    ...
    componentDidMount(){
        Axios.get('http://localhost:3002/info',{params:{
            veh_no:this.props.location.state.search
        }}).then(response=>{
    ...
    clickedHandler=()=>{
        ...
        if(this.state.totalPayment>3){
            obj['veh_no']=this.props.location.state.search
            obj['payment'] = 'Paid'
...
export default OffenceInParticular;